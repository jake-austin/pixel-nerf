...
# DONE, ALL METHODS DONE
class OracleNormalsSpatialEncoder(nn.Module):
    """
    2D (Spatial/Pixel-aligned/local) image encoder
    """
    ...
    def __init__(
        ...
        # self.latent (B, L, H, W)
        self.register_buffer("normals", torch.empty(1, 1, 1, 1), persistent=False)
        # self.normals (B, 3, H, W) these are the normal vectors that we use for ray-based attention

        self.normals_encoder = nn.Sequential(
            [nn.Conv2d(self.latent_size, self.latent_size, 1, padding = 'same'),
            nn.ReLU(),
            nn.Conv2d(self.latent_size, self.latent_size, 1, padding = 'same'),
            nn.ReLU(),
            nn.Conv2d(self.latent_size, self.latent_size, 1, padding = 'same'),
            ...
            nn.Conv2d(self.latent_size, self.latent_size, 1, padding = 'same'),
            nn.ReLU(),
            nn.Conv2d(self.latent_size, 3, 1, padding = 'same')]
        )
    ...
    def index_normals(self, uv, cam_z=None, image_size=(), z_bounds=None):
        """
        Get the predicted normals that we will attend to with the depth oracle
        :param uv (B, N, 2) image points (x,y)
        :param cam_z ignored (for compatibility)
        :param image_size image size, either (width, height) or single int.
        if not specified, assumes coords are in [-1, 1]
    ...
    def forward(self, x):
        ...
        normals = self.normals_encoder(self.latent)
        normals /= torch.linalg.norm(normals, dim = 1)  # make sure our surface normal vectors are unit length
